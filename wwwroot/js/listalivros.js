...
botoesDetalhes.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        const categorias = btn.getAttribute('data-categoria');
...
// -------------------- Lógica de busca --------------------
const inputBusca = document.getElementById('inputBusca'); // pega input da tela
const linhasTabela = document.querySelectorAll('.container_lista_livros tbody tr'); // pega as linhas da tabela, todas elas

if (inputBusca) { // verifica se o input busca tem algo
    inputBusca.addEventListener('input', function () { // toda vez que houver um input nele
        const termo = this.value.toLowerCase();

        linhasTabela.forEach(linha => { // verifica linha por linha, em todas as colunas
            ...
            if (titulo.includes(termo) || categoria.includes(termo) || status.includes(termo)) { // include faz não precisar da palavra exata
                linha.style.display = ''; // mostra os itens
            } else {
                linha.style.display = 'none'; // display none, tira da tela o que o usuário não procura
            }
        });
    });
}
